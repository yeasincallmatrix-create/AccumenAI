...
class Filesystem
{
    ...
    public function replace($path, $content, $mode = null)
    {
        ...
        // Windows-safe atomic replace: rename() fails with "Access is denied (code: 5)" if target is locked by opcache/Apache/antivirus
        if (! @rename($tempPath, $path)) {
            ...
            clearstatcache(true, $path);
            if (! file_exists($path)) {
                throw new ErrorException("Failed to replace file at {$path} - Access is denied (Windows file lock). Check antivirus/opcache lock on storage/framework/views.");
            }
        }
    }
