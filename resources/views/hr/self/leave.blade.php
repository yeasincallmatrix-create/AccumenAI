    ...
    <div class="col-md-4">
        ...
        <div class="admin-card p-3 mt-3">
            ...
            <form method="POST" action="{{ route('hr.self.leave.store') }}" enctype="multipart/form-data">
                ...
                <div class="mb-2"><label class="form-label small">Start</label><x-tdate-input name="start_date" class="form-control form-control-sm" required /></div>
                <div class="mb-2"><label class="form-label small">End</label><x-tdate-input name="end_date" class="form-control form-control-sm" required /></div>
                    ...
                    <tbody>
                        @foreach($history as $h)
                            <tr>
                                ...
                                <td><x-tdate :value="$h->start_date" fallback="Y-m-d" /> → <x-tdate :value="$h->end_date" fallback="Y-m-d" /></td>
                                <td>{{ $h->days_count }}</td>
