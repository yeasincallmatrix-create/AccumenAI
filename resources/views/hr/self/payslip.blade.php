...
<div class="admin-card p-4">
    <div class="row"><div class="col-6"><strong>{{ $hrPayroll->period?->name }}</strong><br><x-tdate :value="$hrPayroll->period?->start_date" fallback="Y-m-d" /> → <x-tdate :value="$hrPayroll->period?->end_date" fallback="Y-m-d" /></div><div class="col-6 text-end"><strong>{{ $employee->display_name }}</strong><br><code>{{ $employee->employee_code }}</code></div></div>
    <hr>
    ...
    <div class="small text-muted mt-2">Status: {{ ucfirst($hrPayroll->status) }} — Generated <x-tdate :value="$hrPayroll->created_at" fallback="Y-m-d" /></div>
</div>
@endsection
