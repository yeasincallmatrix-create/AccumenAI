...
@forelse($rows as $r)<tr><td><x-tdate :value="$r->paid_at" fallback="Y-m-d" /></td><td>{{ $r->supplier?->name }}</td><td>{{ $r->purchaseInvoice?->invoice_number ?? '-' }}</td><td class="text-end">{{ number_format($r->amount,2) }}</td></tr>
