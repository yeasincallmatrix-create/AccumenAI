...
<p><strong>Customer:</strong> {{ $ret->customer?->name }} | <strong>Invoice:</strong> {{ $ret->invoice?->invoice_number }} | <strong>Date:</strong> <x-tdate :value="$ret->return_date" fallback="Y-m-d" /> | <strong>Status:</strong> {{ ucfirst($ret->status) }}</p>
