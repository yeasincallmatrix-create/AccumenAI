...
@section('title', 'Course Requests — AccumenAI')
...
<div class="page-header">
    <div class="page-header-text">
        <h4 class="page-header-title">Course Requests</h4>
        <p class="page-header-desc">Course requests submitted by institutes</p>
    </div>
</div>
...
<div class="filter-card">
    <form class="filter-layout" method="GET" action="{{ route('admin.courses.requests') }}">

        <div class="filter-search-row align-items-end">

            <div class="filter-search" style="flex:1 1 0; min-width:180px;">
                <i class="bi bi-search"></i>
                <input type="text" class="form-control form-control-sm" name="q" placeholder="Search by institute, course name or code..." value="{{ $filters['q'] ?? '' }}">
            </div>
            ...
            <div class="filter-actions">
                ...
                <a class="btn btn-outline-secondary btn-sm" href="{{ route('admin.courses.requests') }}" title="Reset filters"><i class="bi bi-arrow-counterclockwise"></i> Reset</a>
            </div>
...
<div class="admin-card">
    ...
    <div class="table-toolbar">
        <div class="toolbar-info">
            ...
            <span class="text-muted ms-2 d-none d-lg-inline">Review requests from institutes to assign courses.</span>
        </div>
    ...
    <div class="table-responsive">
        <table class="table align-middle mb-0" id="requestsTable">
            <thead>
                ...
                <th data-col="updated_at" @if(!in_array('updated_at', $visibleColumns, true)) style="display:none" @endif>Updated at</th>
                    <th data-col="review_note" @if(!in_array('review_note', $visibleColumns, true)) style="display:none" @endif>Review note</th>
                    <th data-col="reviewed_by" @if(!in_array('reviewed_by', $visibleColumns, true)) style="display:none" @endif>Reviewed by</th>
                    <th data-col="reviewed_at" @if(!in_array('reviewed_at', $visibleColumns, true)) style="display:none" @endif>Reviewed at</th>
                    <th data-col="updated_at" @if(!in_array('updated_at', $visibleColumns, true)) style="display:none" @endif>Updated at</th>
            ...
            <tbody>
                @forelse ($items as $item)
                    <tr>
                        ...
                        <td class="col-check"><input type="checkbox" class="form-check-input row-check" data-name="{{ $item->course->name ?? '' }}"></td>
                        ...
                        <td @if(!in_array('course', $visibleColumns, true)) style="display:none" @endif>
                            ...
                            <div class="text-muted small">{{ $item->course->course_code ?? '' }}</div>
                        </td>
                        ...
                        <td class="text-end text-nowrap col-action" @if(!in_array('action', $visibleColumns, true)) style="display:none" @endif>
                            @if ($item->status === 'pending')
                                <form class="d-inline" method="POST" action="{{ route('admin.courses.requests.action', $item) }}"
                                      data-ajax-action="1" data-confirm="Approve this request?">
                                    @csrf
                                    <input type="hidden" name="action" value="approve">
                                    <button class="btn btn-success btn-sm" type="submit"><i class="bi bi-check-circle"></i> Approve</button>
                                </form>
                                <form class="d-inline" method="POST" action="{{ route('admin.courses.requests.action', $item) }}"
                                      data-ajax-action="1" data-confirm="Reject this request?">
                                    @csrf
                ...
                @empty
                    <tr>
                        <td colspan="20" class="text-center text-muted py-4">No course requests yet.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="mt-4 d-flex flex-column align-items-center gap-2">
        ...
        <span class="text-muted small">{{ $items->total() }} course requests</span>
    </div>
</div>
...
(function () {
    ...
    if (table && colChecks.length) {
        ...
        saveCols = function () {
            ...
            fetch('{{ route('admin.courses.requests-columns') }}', {
                method: 'POST',
                headers: {
    ...
    if (csvBtn) { csvBtn.addEventListener('click', function () { exportTable('course-requests.csv'); }); }
    var excelBtn = document.getElementById('exportExcelBtn');
    if (excelBtn) { excelBtn.addEventListener('click', function () { exportTable('course-requests.xls'); }); }
})();
</script>
@endpush
