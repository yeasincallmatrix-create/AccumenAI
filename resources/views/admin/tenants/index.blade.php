...
@section('title', 'Tenants — AccumenAI')
...
<div class="page-header">
    <div class="page-header-text">
        <h4 class="page-header-title">Tenants</h4>
        <p class="page-header-desc">{{ $items->total() }} registered tenants on the platform</p>
    </div>
...
<div class="filter-card">
    <form class="filter-layout" method="GET" action="{{ route('admin.tenants.index') }}">
        <input type="hidden" name="per_page" value="{{ $perPage ?? 25 }}">

        <div class="filter-search-row align-items-end flex-wrap" style="gap:10px;">
            ...
            <div class="filter-span flex-shrink-0" style="min-width:160px">
                ...
                <select name="country" class="form-select form-select-sm" aria-label="Country">
            ...
            <div class="filter-span flex-shrink-0" style="min-width:180px">
                ...
                <select name="industry" id="industrySelect" class="form-select form-select-sm" aria-label="Industry">
                    <option value="">All Industries</option>
                    @foreach($industries as $key => $label)
                        <option value="{{ $key }}" @selected(request('industry') == $key)>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="filter-span flex-shrink-0" style="min-width:180px">
                ...
                <select name="sub_industry" class="form-select form-select-sm" aria-label="Sub-Industry">
                    <option value="">All Sub Industries</option>
                    @foreach($subIndustries as $subKey => $subLabel)
                        <option value="{{ $subKey }}" @selected(request('sub_industry') == $subKey)>{{ $subLabel }}</option>
                    @endforeach
                </select>
            </div>
            @endif
            ...
            <div class="filter-actions">
                ...
                <a class="btn btn-outline-secondary btn-sm" href="{{ route('admin.tenants.index') }}" title="Reset filters"><i class="bi bi-arrow-counterclockwise"></i> Reset</a>
            </div>
...
<div class="admin-card">
    ...
    <div class="table-toolbar">
        <div class="toolbar-info">
            <span class="badge text-bg-primary badge-soft">{{ $items->total() }} Tenants</span>
            <span class="text-muted ms-2 d-none d-lg-inline">All tenants registered on the platform.</span>
        </div>
    ...
    <div class="table-responsive">
        <table class="table align-middle mb-0" id="institutesTable">
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="20" class="text-center text-muted py-4">No tenants registered yet.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="mt-4 d-flex flex-column align-items-center gap-2">
        ...
        <span class="text-muted small">
            @if($items->total() > 0)
                Showing {{ $items->firstItem() }}–{{ $items->lastItem() }} of {{ $items->total() }} tenants ({{ $perPage ?? 25 }} per page)
            @else
                {{ $items->total() }} tenants
            @endif
        </span>
