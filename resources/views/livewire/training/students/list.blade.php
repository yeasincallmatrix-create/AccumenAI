                ...
                @forelse ($students as $student)
                    <tr>
                        ...
                        @if (in_array('name', $visibleColumns, true))<td>
                            <a class="fw-semibold text-decoration-none" href="{{ route('training.students.show', $student) }}">{{ $student->full_name }}</a>
                        </td>@endif
                        ...
                        @if (in_array('action', $visibleColumns, true))<td class="text-end monetix-print-hidden">
                            @if ($user->hasPermission('students.manage'))
                                ...
                                <form class="d-inline" method="POST" action="{{ route('training.students.destroy', $student) }}"
...
(function () {
    ...
    var KEY = 'accumenai:training:students:columns:v1';
        ...
        try {
            if (window.Livewire) {
                ...
                if (window.Livewire.getByName) {
                    var comps = window.Livewire.getByName('training.student-list');
