...
@section('title', 'Course Master â€” AccumenAI')
...
<div class="page-header">
    <div class="page-header-text">
        ...
        <p class="page-header-desc">{{ $courses->total() }} institute-owned courses â€” author details, pricing and curriculum here.</p>
    </div>
    <div class="page-header-actions d-flex gap-2">
        <a href="{{ route('training.courses.create') }}" class="btn btn-primary btn-sm">
            <i class="bi bi-plus-lg me-1"></i>Add Course
        </a>
        <a href="{{ route('training.batches.index') }}" class="btn btn-outline-primary btn-sm" title="Create batch for any course">
            <i class="bi bi-collection me-1"></i>Add Batch
        </a>
    </div>
</div>

@include('training.courses._tabs', [
    'activeTab' => 'courses',
...
<div class="filter-card">
    <form class="filter-layout" method="GET" action="{{ route('training.courses.index') }}" data-ajax-filter>
        <div class="filter-search-row align-items-end">
            ...
            <div class="filter-actions">
                ...
                <a class="btn btn-outline-secondary btn-sm" href="{{ route('training.courses.index') }}" title="Reset filters"><i class="bi bi-arrow-counterclockwise"></i> Reset</a>
            </div>
        </div>
    </form>
</div>
...
<div class="admin-card" data-ajax-table>
    ...
    <div class="table-toolbar">
        <div class="toolbar-info">
            ...
            <span class="text-muted ms-2 d-none d-lg-inline">Courses your institute owns â€” author details, pricing and curriculum here.</span>
        </div>
            ...
            <tbody>
                @forelse ($courses as $index => $course)
                    <tr class="course-row" data-href="{{ route('training.courses.show', $course) }}" style="cursor:pointer;">
                        ...
                        <td data-col="category">{{ $course->category->name ?? 'â€”' }}</td>
                        <td data-col="fee">{{ number_format($course->fee ?? 0, 0) }}</td>
                        <td data-col="versions" class="text-center">{{ $course->materials_count ?? 0 }}</td>
    ...
    <div class="mt-4 d-flex flex-column align-items-center gap-2" data-ajax-pagination>
        ...
        <span class="text-muted small">
            @if($courses->total() > 0)
                Showing {{ $courses->firstItem() }}â€“{{ $courses->lastItem() }} of {{ $courses->total() }} courses ({{ $courses->perPage() }} per page)
            @else
        ...
        <tbody>
            @foreach ($courses as $course)
                <tr>
                    ...
                    <td data-col="code">{{ $course->course_code ?? 'â€”' }}</td>
                    <td data-col="course">{{ $course->name }}</td>
                    <td data-col="category">{{ $course->category->name ?? 'â€”' }}</td>
                    <td data-col="fee">{{ number_format($course->fee ?? 0, 0) }}</td>
                    <td data-col="versions">{{ $course->materials_count ?? 0 }}</td>
