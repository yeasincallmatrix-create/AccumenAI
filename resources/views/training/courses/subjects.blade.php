...
<div class="admin-card" data-ajax-table>
    ...
    <div class="filter-card">
        <form class="filter-layout" method="GET" action="{{ route('training.courses.subjects.index') }}" data-ajax-filter>
            ...
            <div class="filter-search-row align-items-end">
                ...
                <div class="filter-actions">
                    ...
                    <a class="btn btn-outline-secondary btn-sm" href="{{ route('training.courses.subjects.index') }}"><i class="bi bi-arrow-counterclockwise"></i> Reset</a>
                </div>
    ...
    <div class="table-toolbar">
        ...
        <div class="toolbar-actions">
            @if ($user->hasPermission('training.courses.manage'))
                <a href="{{ route('training.courses.subjects.create') }}" class="btn btn-sm btn-primary d-flex align-items-center gap-2">
                    ...
                    <tr>
                        ...
                        <td data-col="usage" @if(!in_array('usage', $visibleColumns, true)) style="display:none" @endif>
                            <div class="usage-indicator">
                                ...
                                <a href="{{ route('training.courses.subjects.dependencies', $subject) }}" class="btn btn-sm btn-outline-primary p-0 text-decoration-none" title="View dependencies">
                        ...
                        @if ($user->hasPermission('training.courses.manage'))
                            <td class="text-end col-actions">
                                <a href="{{ route('training.courses.subjects.edit', $subject) }}" class="btn btn-sm btn-outline-primary">
                                    <i class="bi bi-pencil"></i> Edit
                                </a>
                                @if (!$subject->trashed())
                                    <form method="POST" action="{{ route('training.courses.subjects.destroy', $subject) }}" class="d-inline" onsubmit="return confirm('{{ mawa_lang('subjects.delete_confirm') }}');">
                                        @csrf
                                        @method('DELETE')
                                ...
                                @else
                                    <form method="POST" action="{{ route('training.courses.subjects.restore', $subject) }}" class="d-inline" onsubmit="return confirm('{{ mawa_lang('subjects.restore_confirm') }}');">
                                        @csrf
