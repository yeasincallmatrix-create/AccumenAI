...
@section('title', ($course ? 'Edit Course â€” AccumenAI' : 'New Course â€” AccumenAI'))
...
<div class="page-header d-flex flex-wrap align-items-start justify-content-between gap-3">
    <div class="page-header-text">
        ...
        <p class="page-header-desc mb-0">Course Master creates an institute-owned course (institute_id is derived from your account â€” never from the form).</p>
    </div>
...
<div class="admin-card">
    <form id="courseMasterForm" method="POST" action="{{ $course ? route('training.courses.update', $course) : route('training.courses.store') }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="row g-3 mb-3">
            ...
            <div class="col-md-4">
                ...
                <select name="level" id="level" class="form-select form-select-sm @error('level') is-invalid @enderror">
                    <option value="basic" {{ old('level', $course->level ?? 'basic') == 'basic' ? 'selected' : '' }}>Basic (à¦¬à§‡à¦¸à¦¿à¦•)</option>
                    <option value="intermediate" {{ old('level', $course->level ?? 'basic') == 'intermediate' ? 'selected' : '' }}>Intermediate (à¦®à¦¾à¦à¦¾à¦°à¦¿)</option>
                    <option value="advanced" {{ old('level', $course->level ?? 'basic') == 'advanced' ? 'selected' : '' }}>Advanced (à¦à¦¡à¦­à¦¾à¦¨à§à¦¸à¦¡)</option>
                </select>
        ...
        <div class="row g-3 mb-3">
            ...
            <div class="col-md-6">
                ...
                <div class="form-text">JPG or WebP (PNG auto-converted), max 5 MB upload â†’ auto-compressed to â‰¤200 KB (WebP/JPG).</div>
            ...
            <div>
                @if ($course)
                    <button type="button" class="btn btn-outline-danger btn-sm" data-confirm-delete
                            data-href="{{ route('training.courses.destroy', $course) }}">
                        <i class="bi bi-trash me-1"></i>Delete course
                    </button>
                @endif
            </div>
            <div class="d-flex gap-2">
                <a href="{{ route('training.courses.index') }}" class="btn btn-outline-secondary btn-sm">Cancel</a>
...
{{-- â”€â”€ Large modal: Add/remove Category â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ --}}
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div id="catListWrap" class="border rounded" style="max-height:420px; overflow-y:auto;">
                    <div id="catListLoader" class="text-center text-muted small py-4"><span class="spinner-border spinner-border-sm me-2"></span>Loadingâ€¦</div>
                        ...
                        <div class="flex-grow-1">
                            ...
                            <div id="catDeleteReassignGroup" class="d-none">
                                ...
                                <select id="catDeleteReassignSelect" class="form-select form-select-sm">
                                    <option value="">â€” Select replacement â€”</option>
                                </select>
...
{{-- â”€â”€ Large modal: Add Sub Category â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ --}}
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div id="subListWrap" class="border rounded" style="max-height:420px; overflow-y:auto;">
                    <div id="subListLoader" class="text-center text-muted small py-4"><span class="spinner-border spinner-border-sm me-2"></span>Loadingâ€¦</div>
                        ...
                        <div class="flex-grow-1">
                            ...
                            <div id="subDeleteReassignGroup" class="d-none">
                                ...
                                <select id="subDeleteReassignSelect" class="form-select form-select-sm">
                                    <option value="">â€” Select replacement sub category â€”</option>
                                </select>
...
<script>
(function () {
    // â”€â”€ keep existing course delete â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ preserve all text inputs across refresh (clear on submit/cancel) â”€â”€
    (function(){
        ...
        document.querySelectorAll('a[href*="training.courses.index"]').forEach(function(a){
            a.addEventListener('click', function(){ try{ sessionStorage.removeItem(KEY);}catch(e){} });
        });
    })();

    var CAT_INDEX = @json(route('training.courses.categories.index'));
    var CAT_STORE = @json(route('training.courses.categories.store'));
    var SUB_INDEX = @json(route('training.courses.sub-categories.index'));
    var SUB_STORE = @json(route('training.courses.sub-categories.store'));
    ...
    // â”€â”€ Category modal logic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    function renderSubAddCategorySelect(cats){
        ...
        if (!sel) return;
        var prev = sel.value;
        sel.innerHTML = '<option value="">â€” Select category â€”</option>';
    ...
    document.addEventListener('click', function(e){
        ...
        if (dbtn) {
            ...
            if (dep > 0) {
                ...
                sel.innerHTML = '<option value="">â€” Select replacement â€”</option>';
    ...
    // â”€â”€ Sub category modal logic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        if (dbtn) {
            ...
            if (dep > 0) {
                ...
                if (!candidates.length) {
                    var o0=document.createElement('option'); o0.value=''; o0.textContent='â€” No replacement available (create one first) â€”'; o0.disabled=true; sel.appendChild(o0);
    ...
    // â”€â”€ main form category -> sub category dynamic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        categorySelect.addEventListener('change', function(){
            ...
            Monetix.request(SUB_INDEX + '?category_id=' + encodeURIComponent(cid), { method:'GET' }).then(function(res){
                ...
                if (subCategorySelect) {
                    subCategorySelect.innerHTML = '<option value="">â€” Not set â€”</option>';
                    subs.forEach(function(s){
                        var opt=document.createElement('option');
            ...
            Monetix.request(SUB_INDEX + '?category_id=' + encodeURIComponent(catSel.value), { method:'GET' }).then(function(res){
                if (res && res.success) {
                    ...
                    if (scSel) {
                        ...
                        var tmp = subs;
                        scSel.innerHTML = '<option value="">â€” Not set â€”</option>';
