...
@section('title', 'Edit Bed — AccumenAI')

@section('content')
<div class="page-header d-flex flex-wrap align-items-center justify-content-between gap-2">
    <div class="page-header-text">
        <h4 class="page-header-title">Edit Bed — {{ $bed->bed_number }}</h4>
    </div>
    <div class="page-header-actions">
        <a class="btn btn-secondary" href="{{ route('medical.beds.show', $bed) }}">
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('medical.beds.update', $bed) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select id="ward_id" name="ward_id" class="form-select @error('ward_id') is-invalid @enderror" required>
                            ...
                            @foreach($wards as $ward)
                                <option value="{{ $ward->id }}" @selected((string) old('ward_id', $bed->ward_id) === (string) $ward->id)>
                    ...
                    <div class="mb-3">
                        ...
                        <select id="status" name="status" class="form-select @error('status') is-invalid @enderror" required>
                            @foreach(['available' => 'Available', 'occupied' => 'Occupied', 'reserved' => 'Reserved', 'maintenance' => 'Maintenance'] as $value => $label)
                                <option value="{{ $value }}" @selected(old('status', $bed->status) === $value)>{{ $label }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-12">
                    <div class="mb-3">
                        ...
                        <textarea id="notes" name="notes" rows="2"
                                  class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $bed->notes) }}</textarea>
            ...
            <div class="mt-3">
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-save me-1"></i>Update Bed
                </button>
                <a href="{{ route('medical.beds.show', $bed) }}" class="btn btn-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>
