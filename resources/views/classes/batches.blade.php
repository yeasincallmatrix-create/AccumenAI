...
@section('title', mawa_lang('classes.tab_batches') . ' — AccumenAI')
...
<div class="page-header">
    <div class="page-header-text">
        <h4 class="page-header-title">{{ mawa_e('classes.tab_batches') }}</h4>
        <p class="page-header-desc">{{ mawa_lang('classes.batches_desc') }} — {{ $institute->name ?? '' }}</p>
    </div>
</div>
        ...
        <div style="width:170px">
            <select name="status" class="form-select">
                ...
                @foreach (['upcoming', 'running', 'completed', 'cancelled'] as $st)
                    <option value="{{ $st }}" @selected((string) $status === $st)>{{ mawa_lang('status.' . $st) }}</option>
                @endforeach
            </select>
        </div>
        <button type="submit" class="btn btn-primary"><i class="bi bi-search me-1"></i>{{ mawa_e('actions.search') }}</button>
        <a class="btn btn-outline-secondary" href="{{ route('classes.batches') }}" title="{{ mawa_e('actions.reset') }}">
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end col-toggle-menu">
                    ...
                    @foreach ([
                        ...
                        <li>
                            <label class="dropdown-item col-toggle-item" for="batch-col-{{ $col }}">
                                <input type="checkbox" id="batch-col-{{ $col }}" class="form-check-input me-2 col-toggle-check" data-col="{{ $col }}" @checked(in_array($col, $visibleColumns, true))>
                                {{ $label }}
                            </label>
                        </li>
                    @endforeach
    ...
    <div class="table-responsive">
        <table class="table align-middle mb-0">
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="8" class="text-center text-muted py-4">{{ mawa_e('classes.batches_empty') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    checks.forEach(function (check) {
        check.addEventListener('change', function () {
            ...
            fetch('{{ route('ui.columns.save') }}', {
                ...
                body: JSON.stringify({ key: 'class_batches', columns: visible })
            });
        });
    });
})();
