...
> **Full "standard list page" spec:** `docs/standard-list-page.md` — the Course
> Assignment page is the canonical implementation (filter form, toolbar,
> columns/preferences, drag, print with full dataset, CSV/Excel export, backend
> contract). Follow it for any new list view.
...
- Row clickability = `.table-row-click` (add `data-row-href="..."` on the `<tr>`
  and include the delegated click handler that navigates to it; clicking a
  link/input inside the row is ignored).
- Empty state = `<tr><td colspan="N" class="text-center text-muted py-4">...`.
- Pagination: `{{ $rows->links('pagination::bootstrap-5') }}` inside a centered
  column footer, **pagination box first, count text below**:

  ```blade
  <div class="mt-3 d-flex flex-column align-items-center gap-2">
      {{ $rows->links('pagination::bootstrap-5') }}
      <span class="text-muted small">{{ $rows->total() }} ...</span>
  </div>
  ```

  Pagination is centered on **every** page, for **every** user.

- Optional per-user show/hide columns = a "Columns" dropdown (candle-btn +
  `col-toggle-menu` / `col-toggle-item`), `data-col` on each `th`/`td`,
  `style="display:none"` when hidden server-side, and a toggle+save script that
  POSTs the visible list to `ui.columns.save` (key `columns_<table>`). See
  `docs/memory.md` → "Show/hide column preferences".
