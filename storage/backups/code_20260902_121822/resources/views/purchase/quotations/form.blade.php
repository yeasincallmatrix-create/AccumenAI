...
@section('title', ($isEdit ? 'Edit' : 'New') . ' Purchase Quotation — AccumenAI')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0">{{ $isEdit ? 'Edit Purchase Quotation' : 'New Purchase Quotation' }} @if($isEdit) <small class="text-muted">{{ $quotation->quotation_number }}</small> @endif</h4>
...
<form method="POST" action="{{ $isEdit ? route('purchase.quotations.update', $quotation) : route('purchase.quotations.store') }}">
    @csrf
        ...
        <div class="card-body">
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <select name="supplier_id" class="form-select" required>
                        ...
                        @php
                            ...
                            $selectedSupplier = old('supplier_id', $quotation?->supplier_id ?? null);
                        @endphp
                ...
                <div class="col-md-4">
                    ...
                    <select name="warehouse_id" class="form-select">
                        ...
                        @php
                            ...
                            $selectedWh = old('warehouse_id', null);
                        @endphp
                ...
                <div class="col-md-3">
                    ...
                    <select name="currency_id" class="form-select">
                        ...
                        @foreach ($currencies as $cur)
                            <option value="{{ $cur->id }}" {{ (string)old('currency_id', $quotation?->currency_id ?? '') === (string)$cur->id ? 'selected' : '' }}>{{ $cur->code }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Reference</label>
                    <input type="text" name="reference" value="{{ old('reference', '') }}" class="form-control" placeholder="REF-001">
                </div>
                <div class="col-md-3">
                    <label class="form-label">Discount Type</label>
                    <select name="discount_type" class="form-select">
                        <option value="fixed" {{ old('discount_type', $quotation?->discount_type ?? 'fixed') === 'fixed' ? 'selected' : '' }}>Fixed</option>
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" step="0.0001" min="0" name="discount_amount" value="{{ old('discount_amount', $quotation?->discount_amount ?? 0) }}" class="form-control">
                </div>
                <div class="col-md-6">
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table mb-0" id="linesTable">
                    ...
                    <tbody>
                        @php
                            $oldLines = old('lines', $quotation?->lines?->toArray() ?? [['description'=>'','quantity'=>1,'unit_price'=>0,'discount_amount'=>0,'discount_type'=>'fixed']]);
                        @endphp
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ $isEdit ? route('purchase.quotations.show', $quotation) : route('purchase.quotations.index') }}" class="btn btn-outline-secondary rounded-pill">Cancel</a>
    </div>
</form>
@endsection
