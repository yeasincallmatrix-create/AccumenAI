...
@section('title', 'Course Assignment — AccumenAI')
...
<style>
    ...
    @media print {
        ...
        #assignmentTablePrint { width: 100%; font-size: 11px; }
        #assignmentTablePrint th, #assignmentTablePrint td { padding: 4px 6px !important; white-space: nowrap; }
    }
</style>
@php
    $statusBadge = [
...
<div class="page-header">
    <div class="page-header-text">
        <h4 class="page-header-title">Course Assignment</h4>
        <p class="page-header-desc">Assign or remove courses for a selected institute.</p>
    </div>
</div>
...
<div class="filter-card">
    <form class="filter-layout" method="GET" action="{{ route('admin.courses.assignment') }}">
        ...
        <div class="filter-search-row align-items-end">
            ...
            <div class="filter-span" style="flex:1 1 0; min-width:180px;">
                ...
                <div class="inst-dropdown" id="instDropdown">
                    <input type="text" class="form-control form-control-sm" id="instSearch" autocomplete="off"
                           placeholder="Search institute..." value="{{ $selectedInstitute->name ?? '' }}">
                    <input type="hidden" name="institute_id" id="instValue" value="{{ $selectedInstitute->id ?? '' }}">
                    <span class="inst-caret"><i class="bi bi-chevron-down"></i></span>
                    <ul class="inst-list" id="instList">
                        @foreach ($institutes as $inst)
                            <li class="inst-item {{ ($selectedInstitute->id ?? null) === $inst->id ? 'active' : '' }}" data-value="{{ $inst->id }}">{{ $inst->name }}</li>
                        @endforeach
                    </ul>
                </div>
            </div>
            ...
            <div class="filter-actions">
                ...
                <a class="btn btn-outline-secondary btn-sm" href="{{ route('admin.courses.assignment') }}" title="Reset filters"><i class="bi bi-arrow-counterclockwise"></i> Reset</a>
            </div>
...
<div class="admin-card">
    ...
    <div class="table-toolbar">
        <div class="toolbar-info">
            <span class="badge text-bg-success badge-soft">{{ $assignedCount }} Assigned</span>
            <span class="badge text-bg-secondary badge-soft ms-1">{{ $notAssignedCount }} Not Assigned</span>
            <span class="text-muted ms-2 d-none d-lg-inline">Check courses then use bulk actions, or use per-row buttons.</span>
        </div>
    ...
    @if ($selectedInstitute === null)
        <div class="alert alert-warning mb-0">No institute available yet. Create an institute before assigning courses.</div>
    @else
    <div class="table-responsive">
        <table class="table align-middle mb-0" id="assignmentTable">
            <thead>
                <tr>
            ...
            <tbody>
                @forelse ($courses as $course)
                    <tr>
                        ...
                        <td class="text-muted" @if(!in_array('code', $visibleColumns, true)) style="display:none" @endif>{{ $course->course_code }}</td>
                        <td @if(!in_array('course', $visibleColumns, true)) style="display:none" @endif>
                            <div class="fw-semibold">
                                <a class="text-decoration-none" href="{{ route('admin.courses.show', ['course' => $course, 'institute_id' => $selectedInstitute->id ?? null, 'industry' => 'education']) }}">
                                    {{ $course->name }}
                                </a>
                        ...
                        <td @if(!in_array('category', $visibleColumns, true)) style="display:none" @endif>{{ $course->category->name ?? '—' }}
                        ...
                        <td @if(!in_array('level', $visibleColumns, true)) style="display:none" @endif>{{ ucwords(str_replace('_', ' ', $course->level)) }}</td>
                        <td @if(!in_array('fee', $visibleColumns, true)) style="display:none" @endif>{{ mawa_currency_symbol($selectedInstitute->country ?? null) }} {{ number_format($course->fee, 0) }}</td>
                        <td @if(!in_array('discount', $visibleColumns, true)) style="display:none" @endif>{{ $course->discount ? number_format($course->discount, 0) : '—' }}</td>
                        <td @if(!in_array('admission_fee', $visibleColumns, true)) style="display:none" @endif>{{ $course->admission_fee ? number_format($course->admission_fee, 0) : '—' }}</td>
                        <td @if(!in_array('exam_fee', $visibleColumns, true)) style="display:none" @endif>{{ $course->exam_fee ? number_format($course->exam_fee, 0) : '—' }}</td>
                        <td @if(!in_array('certificate_fee', $visibleColumns, true)) style="display:none" @endif>{{ $course->certificate_fee ? number_format($course->certificate_fee, 0) : '—' }}</td>
                        <td @if(!in_array('duration', $visibleColumns, true)) style="display:none" @endif>
                            {{ $course->duration_value ? $course->duration_value . ' ' . ucwords(str_replace('_', ' ', $course->duration_type)) : '—' }}
                        </td>
                        <td @if(!in_array('weekly_classes', $visibleColumns, true)) style="display:none" @endif>{{ $course->weekly_classes ?? '—' }}</td>
                        <td @if(!in_array('total_classes', $visibleColumns, true)) style="display:none" @endif>{{ $course->total_classes ?? '—' }}</td>
                        <td @if(!in_array('class_duration', $visibleColumns, true)) style="display:none" @endif>{{ $course->class_duration_minutes ? $course->class_duration_minutes . ' min' : '—' }}</td>
                        <td @if(!in_array('mode', $visibleColumns, true)) style="display:none" @endif>{{ $course->mode ? ucwords(str_replace('_', ' ', $course->mode)) : '—' }}</td>
                        <td @if(!in_array('batch_capacity', $visibleColumns, true)) style="display:none" @endif>{{ $course->batch_capacity_default ?? '—' }}</td>
                        ...
                        <td @if(!in_array('assignment', $visibleColumns, true)) style="display:none" @endif>
                            @if ($course->is_assigned)
                                <span class="badge text-bg-success badge-soft">Assigned</span>
                            @else
                                <span class="badge text-bg-secondary badge-soft">Not Assigned</span>
                            @endif
                        </td>
                        <td class="text-end text-nowrap col-action" @if(!in_array('action', $visibleColumns, true)) style="display:none" @endif>
                            @if ($course->is_assigned)
                                <form class="d-inline" method="POST" action="{{ route('admin.courses.assignment-remove') }}"
                                      data-ajax-action="1" data-confirm="Remove this course from {{ $selectedInstitute->name }}?">
                                    @csrf
                                    <input type="hidden" name="institute_id" value="{{ $selectedInstitute->id }}">
                                    <input type="hidden" name="course_id" value="{{ $course->id }}">
                                    <button class="btn btn-outline-danger btn-sm btn-icon" type="submit" title="Remove"><i class="bi bi-x-circle"></i></button>
                                </form>
                            @else
                                <form class="d-inline" method="POST" action="{{ route('admin.courses.assignment-assign') }}"
                                      data-ajax-action="1">
                                    @csrf
                                    <input type="hidden" name="institute_id" value="{{ $selectedInstitute->id }}">
                                    <input type="hidden" name="course_id" value="{{ $course->id }}">
                                    <button class="btn btn-outline-success btn-sm btn-icon" type="submit" title="Assign"><i class="bi bi-check-circle"></i></button>
                                </form>
                            @endif
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="20" class="text-center text-muted py-4">No courses found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<div class="print-only">
    <table class="table align-middle mb-0" id="assignmentTablePrint">
        <thead>
            <tr>
        ...
        <tbody>
            @foreach ($allCourses as $course)
                <tr>
                    ...
                    <td data-col="fee" @if(!in_array('fee', $visibleColumns, true)) style="display:none" @endif>{{ mawa_currency_symbol($selectedInstitute->country ?? null) }} {{ number_format($course->fee, 0) }}</td>
                    ...
                    <td data-col="assignment" @if(!in_array('assignment', $visibleColumns, true)) style="display:none" @endif>{{ $course->is_assigned ? 'Assigned' : 'Not Assigned' }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
...
(function () {
    ...
    var tbody = document.getElementById('assignmentTable');
    ...
    if (table && colChecks.length) {
        colChecks.forEach(function (check) {
            check.addEventListener('change', function () {
                ...
                var printTable = document.getElementById('assignmentTablePrint');
                if (printTable) {
        ...
        saveCols = function () {
            ...
            fetch('{{ route('admin.courses.assignment-columns') }}', {
                method: 'POST',
                headers: {
    ...
    if (csvBtn) { csvBtn.addEventListener('click', function () { exportTable('course-assignment.csv'); }); }
    var excelBtn = document.getElementById('exportExcelBtn');
    if (excelBtn) { excelBtn.addEventListener('click', function () { exportTable('course-assignment.xls'); }); }
})();
</script>
@endpush