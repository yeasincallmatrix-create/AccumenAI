    ...
    public function up(): void
    {
        if (! Schema::hasTable('purchase_quotations')) {
            Schema::create('purchase_quotations', function (Blueprint $table) {
                ...
                $table->unique(['institute_id', 'quotation_number'], 'uq_pq_number');
                $table->index(['institute_id', 'branch_id', 'status'], 'idx_pq_scope_status');
                $table->index(['institute_id', 'supplier_id'], 'idx_pq_supplier');
                $table->index(['institute_id', 'quotation_date'], 'idx_pq_date');
            });
        }
        ...
        if (! Schema::hasTable('purchase_quotation_lines')) {
            Schema::create('purchase_quotation_lines', function (Blueprint $table) {
                ...
                $table->index(['quotation_id', 'sort_order'], 'idx_pq_lines_order');
            });
        }
    }
