...
class ReceivableList extends DataTable
{
    protected const VIEW = 'livewire.receivables.list';
    ...
    {
        return ['name', 'receivable', 'payable'];
    }
    ...
    public function getRows(): LengthAwarePaginator
    {
        ...
        $query = Party::query()
            ...
            ->whereIn('parties.type', ['customer', 'both'])
        ...
        // Aging buckets (DB-level)
        ->selectRaw("COALESCE(SUM(CASE WHEN coa.is_receivable = 1 AND DATEDIFF({$asOfLiteral}, je.journal_date) <= 30 THEN je.debit - je.credit ELSE 0 END), 0) AS aging_current")
        ->selectRaw("COALESCE(SUM(CASE WHEN coa.is_receivable = 1 AND DATEDIFF({$asOfLiteral}, je.journal_date) BETWEEN 31 AND 60 THEN je.debit - je.credit ELSE 0 END), 0) AS aging_31_60")
        ->selectRaw("COALESCE(SUM(CASE WHEN coa.is_receivable = 1 AND DATEDIFF({$asOfLiteral}, je.journal_date) BETWEEN 61 AND 90 THEN je.debit - je.credit ELSE 0 END), 0) AS aging_61_90")
        ->selectRaw("COALESCE(SUM(CASE WHEN coa.is_receivable = 1 AND DATEDIFF({$asOfLiteral}, je.journal_date) > 90 THEN je.debit - je.credit ELSE 0 END), 0) AS aging_91_plus")
