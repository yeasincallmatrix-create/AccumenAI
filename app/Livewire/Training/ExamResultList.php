...
{
    protected const VIEW = 'livewire.training.exams.results';
    ...
    {
        $user = auth()->user();
        $this->visibleColumns = $user->preference('columns_training_exam_results', [
            'serial', 'student', 'student_id', 'batch', 'total_marks', 'obtained', 'percentage', 'status', 'published_at',
        ]);
    ...
    {
        // TrainingBatchResult has no course/certificate relations — student + batch only.
        return TrainingBatchResult::query()
            ->with(['student:id,first_name,last_name,student_id_number', 'batch:id,name,batch_code']);
    }
    ...
    protected function applyFilter(Builder $query, string $key, mixed $value, array $config): void
    {
        match ($key) {
            ...
            // DB column is `status`; `result_status` is a read accessor only.
            'result_status' => $query->where('status', $value),
            default => null,
        };
    }
    ...
    public function getRows(): \Illuminate\Pagination\LengthAwarePaginator
    {
        ...
        if (filled($this->filters['result_status'] ?? '')) {
            $query->where('status', $this->filters['result_status']);
        }
    ...
    public function saveColumns(): void
    {
        ...
        if ($user && method_exists($user, 'preference')) {
            $user->setPreference('columns_training_exam_results', $this->visibleColumns);
        }
    }
