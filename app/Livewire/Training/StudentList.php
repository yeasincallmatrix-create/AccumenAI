...
{
    protected const VIEW = 'livewire.training.students.list';
    ...
    {
        $user = auth()->user();
        $stored = $user?->preference('columns_training_students', self::DEFAULT_COLUMNS);
        if (! is_array($stored)) {
    ...
    protected function baseQuery(): Builder
    {
        ...
        $query = TrainingStudent::query()->with('branch');
        if ($instituteId) {
            $query->where('training_students.institute_id', $instituteId);
        } else {
    ...
    public function saveColumns(): void
    {
        ...
        if ($user && method_exists($user, 'preference')) {
            ...
            $user->setPreference('columns_training_students', $this->visibleColumns);
        }
    }
