...
use App\Services\Medical\MedicineImportAnalyzer;
...
class MedicineController extends MedicalController implements HasMiddleware
{
    public static function middleware(): array
    {
        return [
            ...
            new Middleware('permission:medical_medicines.create', only: ['create', 'store', 'importUpload', 'importReview', 'importConfirm', 'importCancel']),
    ...
    public function quickStore(Request $request)
    {
        ...
        $code = $request->input('code') ?: null; // null → auto-generated by model
    ...
    public function store(MedicineRequest $request)
    {
        ...
        // Empty string → null → sequential auto-assignment by the model.
    ...
    public function import(Request $request)
    {
        ...
        try {
            while (($row = fgetcsv($handle)) !== false) {
                ...
                // Empty code → null → sequential auto-assignment by the model.
                // Legacy MED- codes in the file are preserved as-is.
                $code = ! empty($data['code']) ? trim($data['code']) : null;

                // Validate code format when provided: 4–6 digit numeric
                // (new convention) or legacy MED- prefix. Anything else
                // (e.g. RX- prescription codes) is skipped with an error —
                // the rest of the file still imports.
                if ($code !== null && $code !== '') {
                    if (! preg_match('/^([0-9]{4,6}|MED-[A-Za-z0-9\-]+)$/', $code)) {
                        $errors[] = "Row {$rowNum}: invalid code '{$code}'. Must be 4-6 digits or legacy MED- format.";
                        $skipped++;
                        continue;
                    }
                    ...
                    $codeExists = Medicine::where('institute_id', $instituteId)
                        ...
                        ->exists();
                    if ($codeExists) {
                        $errors[] = "Row {$rowNum}: code '{$code}' already in use in your institute.";
                        $skipped++;
                        continue;
                    }
                } else {
                    // Empty code → will be auto-generated by model hook
                    $code = null;
                }
    ...
    /**
     * Two-phase import, phase 1: upload CSV and analyze.
     * Stores the analysis as a pending batch and redirects to review.
     * The legacy single-pass import() above is preserved unchanged.
     */
    public function importUpload(Request $request, MedicineImportAnalyzer $analyzer)
    {
        $request->validate([
            'csv_file' => 'required|file|mimes:csv,txt|max:5120',
        ]);
        ...
        $analysis = $analyzer->analyze($rows, $instituteId);

        $batch = MedicineImportBatch::create([
            'batch_id' => (string) Str::uuid(),
            'institute_id' => $instituteId,
            'uploaded_by' => auth()->id(),
            'original_filename' => $file->getClientOriginalName(),
            'total_rows' => count($rows),
            'clean_rows' => count($analysis['clean']),
            'conflict_rows' => count($analysis['conflicts']),
            'error_rows' => count($analysis['errors']),
            'parsed_data' => $analysis,
            'status' => MedicineImportBatch::STATUS_PENDING,
        ]);

        return redirect()->route('medical.pharmacy.medicines.import.review', $batch->batch_id);
    }

    /**
     * Two-phase import, phase 2: review page with conflicts.
     */
    public function importReview(string $batchId)
    {
        $batch = MedicineImportBatch::where('batch_id', $batchId)
            ->where('institute_id', $this->instituteId())
            ->where('uploaded_by', auth()->id())
            ->firstOrFail();

        return view('medical.medicines.import-review', compact('batch'));
    }

    /**
     * Two-phase import, phase 3: confirm and apply resolutions.
     */
    public function importConfirm(Request $request, string $batchId, MedicineImportAnalyzer $analyzer)
    {
        $batch = MedicineImportBatch::where('batch_id', $batchId)
            ->where('institute_id', $this->instituteId())
            ->where('uploaded_by', auth()->id())
            ->where('status', MedicineImportBatch::STATUS_PENDING)
            ->firstOrFail();

        $resolutions = $request->input('resolutions', []);
        $analysis = $batch->parsed_data;

        $applied = $analyzer->applyResolutions($analysis['conflicts'] ?? [], $resolutions);

        $allRows = array_merge($analysis['clean'] ?? [], $applied['rows']);
        ...
        try {
            foreach ($allRows as $row) {
                $resolution = $row['_resolution'] ?? 'create';
                $existingId = $row['_existing_id'] ?? null;

                if ($resolution === 'update' && $existingId) {
                    $existing = Medicine::find($existingId);
                    if ($existing && (int) $existing->institute_id === (int) $batch->institute_id) {
                        $existing->update($this->buildUpdatePayload($row));
                        $imported++;
                        continue;
                    }
                }
                ...
                if ($resolution === 'create_new' && ! empty($code)) {
                    $exists = Medicine::where('institute_id', $batch->institute_id)
                        ->where('code', $code)
                        ->whereNull('deleted_at')
                        ->exists();
                    if ($exists) {
                        $code = null; // auto-generate sequential
                    }
                }

                try {
                    Medicine::create(array_merge(
                        $this->buildCreatePayload($row, $batch->institute_id),
                        ['code' => $code]
                    ));
                    $imported++;
                } catch (\Throwable $e) {
                    $errors[] = "Row {$row['_row_num']}: ".$e->getMessage();
                }
            }

            $batch->update([
                'status' => MedicineImportBatch::STATUS_CONFIRMED,
                'confirmed_at' => now(),
        ...
        return redirect()->route('medical.pharmacy.medicines.index')
            ->with('success', "Import complete: {$imported} imported, {$skipped} skipped, ".count($errors).' errors.');
    }

    public function importCancel(string $batchId)
    {
        $batch = MedicineImportBatch::where('batch_id', $batchId)
            ->where('uploaded_by', auth()->id())
            ->firstOrFail();
        $batch->update(['status' => MedicineImportBatch::STATUS_CANCELLED]);

        return redirect()->route('medical.pharmacy.medicines.import.form')
            ->with('info', 'Import cancelled.');
    }

    protected function buildCreatePayload(array $row, int $instituteId): array
    {
        // Empty strings become null: several columns (notably dgda_code)
        // carry UNIQUE indexes where '' would collide across rows.
        $null = fn ($key) => isset($row[$key]) && trim((string) $row[$key]) !== '' ? trim((string) $row[$key]) : null;
        ...
        return [
            ...
            'dosage_form' => ucfirst(strtolower(trim($row['dosage_form']))),
            'route' => $null('route'),
            'unit' => $null('unit'),
            'pack_size' => ! empty($row['pack_size']) ? (int) $row['pack_size'] : null,
            'manufacturer' => $null('manufacturer'),
            'category' => $null('category'),
            'selling_price' => ! empty($row['selling_price']) ? (float) $row['selling_price'] : 0,
            'reorder_level' => ! empty($row['reorder_level']) ? (int) $row['reorder_level'] : 10,
            'dgda_code' => $null('dgda_code'),
            'is_active' => isset($row['is_active']) ? (bool) $row['is_active'] : true,
        ];
    }

    protected function buildUpdatePayload(array $row): array
    {
        $null = fn ($key) => isset($row[$key]) && trim((string) $row[$key]) !== '' ? trim((string) $row[$key]) : null;
