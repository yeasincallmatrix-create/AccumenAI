    ...
    public function index(Request $request): View
    {
        ...
        if ($selectedExamId) {
            ...
            if ($selectedExam && (int) $selectedExam->institute_id === $instituteId) {
                ...
                $examResults = TrainingExamResult::where('exam_id', $selectedExamId)
                    ->select('student_id', DB::raw('SUM(marks_obtained) as obtained_marks'))
                    ->groupBy('student_id')
                    ->get()
                    ->keyBy('student_id');

                $examForMarks = TrainingExam::find($selectedExamId);
    ...
    public function store(Request $request)
    {
        ...
        foreach ($request->marks as $traineeId => $obtained) {
            ...
            TrainingExamResult::updateOrCreate(
                ['exam_id' => $exam->id, 'student_id' => $traineeId],
                ['marks_obtained' => $obtained, 'result_status' => $status, 'institute_id' => $instituteId]
            );
        }
