...
class PurchaseLifecycleHardeningTest extends TestCase
{
    ...
    private function institute(?Country $c=null): Institute { $c??=$this->country(); $i=Institute::create(['name'=>'P9 '.uniqid(),'slug'=>'p9-'.uniqid(),'industry'=>'retail','country'=>$c->name,'country_id'=>$c->id,'status'=>'active']); app(AccountingSetupService::class)->setupForInstitute($i->id); app(\App\Services\ModuleAccessService::class)->enableModule($i, 'purchase'); return $i; }
