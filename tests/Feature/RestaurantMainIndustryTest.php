...
class RestaurantMainIndustryTest extends TestCase
{
    ...
    // 7 - 79 restaurant permissions exist.

    public function test_restaurant_permissions_are_seeded(): void
    {
        $declared = RestaurantPermissionSeeder::permissions();

        $this->assertCount(79, $declared, '79 declared permissions');
        $this->assertCount(79, array_unique(array_column($declared, 'slug')), 'declared slugs unique');
        ...
        $this->assertCount(79, $rows, '79 restaurant permissions persisted');
        $this->assertCount(79, $rows->pluck('slug')->unique(), 'persisted slugs unique');
        $this->assertCount(
            79,
